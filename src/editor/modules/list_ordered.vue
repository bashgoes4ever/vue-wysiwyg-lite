<template>
  <form @submit.prevent="insertList">
    <label>
      <span>Start index</span>
      <input ref="num" type="number" required="" style="width: 100%" v-model="number">
    </label>
...
export default {
  ...
  methods: {
    insertList() {
      this.$emit("exec", "insertHTML", `<ol start=${this.number}><li></li></ol>`);
  ...
  created() {
    bus.on(this.uid + "_show_dashboard_list_ordered", () => {
      console.log('bus:', bus)
      this.$nextTick(() => {
        this.$refs.num.focus();
      });
